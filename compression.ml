...
let compress story =
  ...
  let rec aux acc i c =
    if i = memory_length then
      ...
      let original_byte = Story.read_byte original_story i in
      let current_byte = Story.read_byte story i in
...
let apply_uncompressed_changes story (Uncompressed uncompressed) =
  ...
  let rec aux index acc =
    if index >= length then
      ...
      let orig_byte = Story.read_byte original_story index in
      let new_story =
        if new_byte = orig_byte then acc
        else Story.write_byte acc index new_byte in
  ...
  let rec aux index_change index_mem acc =
    if index_change >= length then
      ...
      if b = 0 then
        ...
        let orig_byte = Story.read_byte original_story index_mem in
        let new_byte = b lxor orig_byte in
        let new_story = Story.write_byte acc index_mem new_byte in
